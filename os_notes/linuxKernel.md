...
通用文件系统接口(VFS)使用户可以直接调用open, read, write系统调用而无需考虑具体的文件系统。

### Unix文件系统

使用了4种相关的概念：文件，目录项，索引节点和安装点。文件系统被安装在一个特定的安装点上，所有已安装文件系统都作为根文件系统树的枝叶出现在系统中。

unix将文件的相关信息和文件本身加以区分，文件相关信息被单独存储在一个数据结构中，称为索引节点(`index node, or inode`)。这些信息和文件系统的控制信息密切相关，文件系统的控制信息存储在超级块中，超级块是一种包含文件系统信息的数据结构。在磁盘上，文件信息按照索引节点形式存储在单独的块中，控制信息被集中存储在磁盘的超级块中。

VFS中有4个主要的对象类型：
- 超级块对象，代表一个具体已安装文件系统
- 索引节点对象，代表一个具体文件
- 目录项对象，代表一个目录项，是路径的一个组成部分
- 文件对象，代表进程打开的文件（目录也作为文件处理）

#### 超级块对象

各个文件系统都必须实现超级块对象，该对象用于存储特定文件系统的信息，通常对应于存放在特定扇区的文件系统超级块或文件系统控制块。

#### 索引节点对象

包含了内核在操作文件或者目录时所需要的全部信息。对于UNIX，可以从磁盘的索引节点直接读取。一个索引节点代表文件系统中的一个文件（只有当文件被访问时，索引节点才会在内存中创建）。可以是文件，设备，管道。

#### 目录项对象

在路径/bin/vi中，bin,vi都是文件，路径中的每个组成都有一个索引节点对象表示。为了方便面查找操作，引入了目录项(dentry)的概念。/, bin, vi 都是目录项对象，在一个路径中，每一个部分都是目录项对象，代替了传统的字符串比较。目录项对象没有对应的磁盘数据结构，VFS根据字符串形式的路径名现场创建。目录项对象有三种状态：
- 被使用：对应一个有效的索引节点，该对象存在一个或多个使用者。
- 未被使用：对应一个有效的索引节点，但是没有对象在使用。
- 负状态：没有对应的有效索引节点，因为索引节点被删除了，或者路径不正确

内核将目录项对象缓存在目录项缓存中，以加快路径的解析目录项缓存也包括三个部分：
- 被使用的目录项链表。
- 最近被使用的双向链表。包含未被使用的和负状态的目录项对象。（lru算法更新）
- 散列表和对应的hash func,快速的将给定路径解析为相关的目录项对象（哈希表快速定位）。
  
#### 文件对象

文件对象表示进程已打开的文件，该对象由open系统调用创建，close撤销。同一个文件可能有多个文件对象，文件对象仅仅在进程观点上代表已打开文件，反过来指向目录项对象，再反过来指向索引节点。一个文件对应的文件对象可能不是唯一的，但是对应的目录项对象和索引节点对象是唯一的。


## 进程地址空间

`内核`使用内存描述符结构体来表示进程的地址空间，其中有这样的几个字段，mm_user是正在使用该地址的进程数据，mm_count是该结构体的主引用计数。当mm_count的值为0时，该结构体就被撤销。

mmap和mm_rb两个不同的数据结构描述对象是相同的，即该地址空间中的全部内存区域，前者以链表形式存放，后者以红黑树形式存放。

所有的mm_struct结构都通过自身的mmlist字段链接在一个双向链表中。在进程的PCB中，mm字段存放进程的内存描述符，fork利用copy_mm复制父进程的聂村描述符，而子进程的mm_struct结构体实际上是通过allocate_mm从mm_cachep slab缓存中分配得到的。通常每个进程都有唯一的mm_struct，即唯一的地址空间，如果父进程希望与子进程共享地址空间，可以在调用clone时，设置CLONE_VM标志，也就是又产生了一个线程。只要tsk->mm = current->mm就好了。